...
std::vector<int64_t> free_binding_ids;
...
void release_argument_binding(int64_t binding_id) {
  std::shared_ptr<ArgumentBinding> binding;
  {
    ...
    auto& slot = bindings[static_cast<size_t>(binding_id)];
...
int64_t create_argument_binding(
  ...
  std::lock_guard<std::mutex> guard(binding_mutex);
  if (!free_binding_ids.empty()) {
    const int64_t id = free_binding_ids.back();
    free_binding_ids.pop_back();
    bindings[static_cast<size_t>(id)] = std::move(binding);
    return id;
  }
...
int64_t create_icb(
  ...
  if (!free_graph_ids.empty()) {
    const int64_t id = free_graph_ids.back();
    free_graph_ids.pop_back();
    graphs[static_cast<size_t>(id)] = std::move(graph);
    return id;
  }
...
void release_icb(int64_t graph_id) {
  std::shared_ptr<ICBGraph> graph;
  {
    ...
    auto& slot = graphs[static_cast<size_t>(graph_id)];
    if (slot == nullptr) return;
    graph = std::move(slot);
