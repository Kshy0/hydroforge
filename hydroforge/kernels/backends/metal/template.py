...
def _split_template_source(
    ...
    if len(names) != len(set(names)):
        raise ValueError(f"Metal template source has duplicate bodies: {names}")
    try:
        selected = names.index(spec.name)
    except ValueError as error:
        raise ValueError(
            f"Metal template source has no body for {spec.name!r}; available={names}"
        ) from error
...
class SpecMetalTemplateDispatcher:
        ...
        *,
        ...
        for fragment, scope, structure in (
            ...
            if forbidden or re.search(structure, fragment):
                raise ValueError(
                    f"Metal template {scope}; forbidden wrapper syntax={forbidden}"
                )
        ...
        execution_axes = (
            {execution_size_key}
            if isinstance(execution_size_key, str)
            else set(execution_size_key)
        )
        ...
        self._validated_signatures: dict[tuple[Any, ...], tuple[Any, ...]] = {}
        ...
        for name in self.spec.parameters:
            if name in self.spec.compile_time:
                result.append(
                    (
                        "constant",
                        COMPILE_SCALAR_TYPES[self.spec.compile_time[name]],
                    )
                )
                ...
                if buffer_access_semantics(
                    self.spec.buffers[name]
                ).atomic and native_type not in {"float", "int"}:
                    raise TypeError(
                ...
                result.append(
                    (
                        "scalar",
                        RUNTIME_SCALAR_TYPES[self.spec.runtime_scalars[name]],
                    )
                )
        return tuple(result)
    ...
    def _validate_specialization_input(
        ...
        *,
        ...
        self._validated_signatures[self._signature_key(buffer_dtypes)] = signature
    ...
    def _render_source(self, signature: tuple[Any, ...]) -> str:
        ...
        for name, (_kind, native_type) in zip(
            ...
            if name in self.spec.buffers:
                ...
                dtype = native_type
                qualifier = "constant"
            fields.append(f"    {qualifier} {dtype}* {name} [[id({len(fields)})]];")
        constant_source = "\n".join(constants)
        field_source = "\n".join(fields)
        group_source = (
            ""
            if self.fixed_group_size is None
            else f"constant constexpr uint HF_BLOCK_SIZE = {self.fixed_group_size};"
        )
        return f"""
...
"""
    ...
    def specialize(
        ...
        *,
        ...
        signature = self._validated_signatures[self._signature_key(buffer_dtypes)]
...
class _SpecMetalDispatcherDeclaration(HydroForgeModel):
    ...
    def _build(self):
        try:
            spec = resolve_factory_spec(self.spec, factory="make_spec_metal_dispatcher")
