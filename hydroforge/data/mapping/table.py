...
class LocalMapping:
    ...
    def to_torch(self, *, device: Any, dtype: Any) -> Any:
        """Materialize the canonical ``active_source x target`` sparse tensor."""
        import torch

        matrix = self.source_to_target.tocoo()
        indices = torch.from_numpy(np.stack((matrix.row, matrix.col))).to(
            device=device, dtype=torch.int64,
        )
        values = torch.from_numpy(matrix.data).to(device=device, dtype=dtype)
        with torch.sparse.check_sparse_tensor_invariants(enable=True):
            tensor = torch.sparse_coo_tensor(
                indices, values, size=matrix.shape, device=device, dtype=dtype,
            )
