...
from hydroforge.io.spatial_mapping import (MappingTable, RegularGrid,
                                           TargetSupport,
    ...
    *,
    ...
    if desired_ids is not None:
        ...
        if not np.all(present):
            print(
                f"Warning: {int((~present).sum())} IDs from parameter_nc are not "
                "present in current map; they will be skipped."
            )
        target_ids = desired_ids[present]
    else:
    ...
    *,
    ...
    with nc.Dataset(nc_path, "w", format="NETCDF4") as ds:
        ...
        out_var = ds.createVariable(
            out_name, dtype_nc, dims, zlib=True, complevel=int(complevel)
        )
