...
class HipCodegenMixin:
    """HIP C++ kernel code generation for statistics aggregation on AMD GPUs."""

    # ========================================================================
    # HIP C++ code generation
    # ========================================================================

    def _hip_dtype_str(self: StatisticsAggregator, var_name: str) -> str:
        """Return the HIP C++ scalar type string for a variable's tensor dtype."""
        import torch
    ...
    def _hip_ptr_type(self: StatisticsAggregator, var_name: str) -> str:
        """Return the C++ pointer type for a variable (e.g. 'float*')."""
        return self._hip_dtype_str(var_name) + "*"

    def _hip_emit_val_load(self: StatisticsAggregator, var_name: str,
                            lines: list, emitted: set, indent: str,
                            idx_expr: str = "idx") -> str:
        """Emit HIP C++ code to load a variable value (handling virtuals).
        ...
        ctype = self._hip_dtype_str(var_name)
        ...
        if cat == 'virtual':
            ...
            for t in toks:
                if t in self._field_registry or t in self._tensor_registry:
                    dep_val = self._hip_emit_val_load(t, lines, emitted, indent, idx_expr)
    ...
    def _generate_hip_kernel_for_group(self: StatisticsAggregator,
                                        hip_lines: list, cpp_lines: list,
                                        save_idx: str, var_list: list,
                                        tensor_info: dict) -> None:
        """Generate HIP __global__ kernel + C++ launcher for one save_idx group."""
        ...
        # ---- Generate HIP kernel ----
        I = "    "
        hip_lines.append(f'__global__ void {kernel_name}(')
        hip_lines.append(f'    const int* p_{safe_save},')
        for inp in sorted_inputs:
            safe_inp = self._get_safe_name(inp)
            if safe_inp == safe_save:
                continue
            ctype = self._hip_dtype_str(inp)
            hip_lines.append(f'    const {ctype}* p_{safe_inp},')
        for _, ctype, pname in out_tensors:
            hip_lines.append(f'    {ctype}* {pname},')
        hip_lines.append(f'    const float* p_weight_ptr, const float* p_total_weight_ptr, const float* p_num_macro_steps_ptr,')
        hip_lines.append(f'    const int* p_sub_step_ptr, const int* p_num_sub_steps_ptr, const int* p_flags_ptr,')
        hip_lines.append(f'    const int* p_macro_step_index_ptr,')
        hip_lines.append(f'    int n_saved_points, int stride_input')
        if has_2d:
            hip_lines[-1] += ', int n_levels'
        hip_lines.append(') {')
        hip_lines.append(f'{I}int tid = blockIdx.x * blockDim.x + threadIdx.x;')
        hip_lines.append(f'{I}if (tid >= n_saved_points) return;')
        hip_lines.append(f'{I}int idx = p_{safe_save}[tid];')
        hip_lines.append(f'')
        hip_lines.append(f'{I}float weight = *p_weight_ptr;')
        hip_lines.append(f'{I}float total_weight = *p_total_weight_ptr;')
        hip_lines.append(f'{I}float num_macro_steps = *p_num_macro_steps_ptr;')
        hip_lines.append(f'{I}int sub_step = *p_sub_step_ptr;')
        hip_lines.append(f'{I}int num_sub_steps = *p_num_sub_steps_ptr;')
        hip_lines.append(f'{I}int flags = *p_flags_ptr;')
        hip_lines.append(f'{I}int macro_step_index = *p_macro_step_index_ptr;')
        hip_lines.append(f'')
        needed_bools = self._analyze_needed_booleans()
        if needed_bools:
            hip_lines.append(f'{I}// Compute boolean flags from sub_step, num_sub_steps, flags')
            if 'is_inner_first' in needed_bools:
                hip_lines.append(f'{I}bool is_inner_first = (flags & 1) && (sub_step == 0);')
            if 'is_inner_last' in needed_bools:
                hip_lines.append(f'{I}bool is_inner_last = ((flags >> 1) & 1) && (sub_step == num_sub_steps - 1);')
            if 'is_middle' in needed_bools:
                hip_lines.append(f'{I}bool is_middle = (sub_step == num_sub_steps / 2);')
            if 'is_outer_first' in needed_bools:
                hip_lines.append(f'{I}bool is_outer_first = ((flags >> 2) & 1) && is_inner_last;')
            if 'is_outer_last' in needed_bools:
                hip_lines.append(f'{I}bool is_outer_last = ((flags >> 3) & 1) && is_inner_last;')
        hip_lines.append('')

        if num_trials > 1:
            hip_lines.append(f'{I}for (int t = 0; t < {num_trials}; t++) {{')
            I2 = I + "    "
        else:
            hip_lines.append(f'{I}// Single trial')
            hip_lines.append(f'{I}const int t = 0;')
            I2 = I

        # -- 1D variables --
        if dims_1d:
            hip_lines.append(f'{I2}// === 1D variables ===')
            emitted = set()
            for var in dims_1d:
                self._hip_emit_val_load(var, hip_lines, emitted, I2, idx_expr="t * stride_input + idx" if num_trials > 1 else "idx")
            ...
            for var in dims_1d:
                ...
                for op in ops:
                    ...
                    if len(op_parts) > 1:
                        ...
                        hip_lines.append(f'{I2}// Compound {op} for {safe_var}')
                        if is_arg:
                            arg_type = outer_base
                    ...
                    # ---- Simple ops ----
                    out_ptr = f"p_{safe_var}_{op}"
                    hip_lines.append(f'{I2}// {op} for {safe_var}')
                    ...
                    if op == 'mean':
                        ...
                        hip_lines.append(f'{I2}if (is_inner_last) {{ {out_ptr}[{out_idx}] = {var_val}; }}')
                    elif op == 'first':
                        hip_lines.append(f'{I2}if (is_inner_first) {{ {out_ptr}[{out_idx}] = {var_val}; }}')
                    elif op == 'mid':
                        hip_lines.append(f'{I2}if (is_middle) {{ {out_ptr}[{out_idx}] = {var_val}; }}')

        # -- 2D variables --
        if dims_2d:
            hip_lines.append(f'{I2}// === 2D variables ===')
            for var in dims_2d:
                safe_var = self._get_safe_name(var)
                ctype = self._hip_dtype_str(var)
                for op in self._variable_ops[var]:
                    out_key = f'{var}_{op}'
                    out_ptr = f"p_{safe_var}_{op}"
                    hip_lines.append(f'{I2}for (int level = 0; level < n_levels; level++) {{')
                    ...
                    hip_lines.append(f'{I2}    {ctype} val_2d = p_{safe_var}[{idx_2d}];')
                    if op == 'mean':
                        hip_lines.extend([
                        ...
                        hip_lines.append(f'{I2}    if (is_inner_last) {{ {out_ptr}[{out_2d}] = val_2d; }}')
                    elif op == 'first':
                        hip_lines.append(f'{I2}    if (is_inner_first) {{ {out_ptr}[{out_2d}] = val_2d; }}')
                    elif op == 'mid':
                        hip_lines.append(f'{I2}    if (is_middle) {{ {out_ptr}[{out_2d}] = val_2d; }}')
                    hip_lines.append(f'{I2}}}')

        if num_trials > 1:
            hip_lines.append(f'{I}}}')
        hip_lines.append('}')
        hip_lines.append('')

        # ---- Generate C++ launcher using hipLaunchKernelGGL ----
        fwd_params = []
        ...
        cpp_lines.append(f'    const auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA();')

        # Build hipLaunchKernelGGL call
        launch_args = [f'        t_{safe_save}.data_ptr<int>()']
        for inp in sorted_inputs:
            safe_inp = self._get_safe_name(inp)
            if safe_inp == safe_save:
                continue
            ctype = self._hip_dtype_str(inp)
            launch_args.append(f'        t_{safe_inp}.data_ptr<{ctype}>()')
        for state_key, ctype, pname in out_tensors:
            safe_key = self._get_safe_name(state_key)
            launch_args.append(f'        t_{safe_key}.data_ptr<{ctype}>()')
        launch_args.append(f'        t_weight.data_ptr<float>(), t_total_weight.data_ptr<float>(), t_num_macro_steps.data_ptr<float>()')
        launch_args.append(f'        t_sub_step.data_ptr<int>(), t_num_sub_steps.data_ptr<int>(), t_flags.data_ptr<int>()')
        launch_args.append(f'        t_macro_step_index.data_ptr<int>()')
        last_scalar = '        n_saved_points, stride_input'
        if has_2d:
            last_scalar += ', n_levels'
        launch_args.append(last_scalar)

        cpp_lines.append(f'    hipLaunchKernelGGL({kernel_name}, dim3(grid), dim3(bs), 0, stream,')
        for i, arg in enumerate(launch_args):
            suffix = ',' if i < len(launch_args) - 1 else ''
            cpp_lines.append(f'{arg}{suffix}')
        cpp_lines.append(f'    );')
        cpp_lines.append('}')
        cpp_lines.append('')
    ...
    def _generate_hip_aggregator_function(
        self: StatisticsAggregator,
    ) -> None:
        """Generate and compile a native HIP C++ aggregation kernel for AMD GPUs.

        Generates ``__global__`` kernels with native HIP API
        (``hipLaunchKernelGGL``, ``at::hip::getCurrentHIPStreamMasqueradingAsCUDA``),
        compiles via ``torch.utils.cpp_extension.load_inline``
        (which auto-hipifies the ``cuda_sources``), and wraps in a
        Python function with the same interface as other backends.
        """
        import hashlib

        from torch.utils.cpp_extension import load_inline

        extra_hip_cflags = ['-O3', '-ffast-math']
        ...
        # HIP kernels + launchers compiled by hipcc
        hip_lines = [
            '// Auto-generated HIP aggregation kernels for hydroforge statistics',
        ...
        for save_idx, var_list in grouped_by_save_idx.items():
            meta = self._generate_hip_kernel_for_group(
                hip_lines, launcher_lines, save_idx, var_list, tensor_info
            )
        ...
        hip_lines.extend(launcher_lines)
        hip_src = '\n'.join(hip_lines)
        ...
        # Compile
        unique = hashlib.md5((hip_src + cpp_src).encode()).hexdigest()[:8]
        module_name = f"hydroforge_aggr_hip_{unique}"

        build_dir = None
        if self.output_dir is not None:
            from pathlib import Path
            build_dir = str(Path(self.output_dir) / "hip_build")
        ...
        compiled_mod = load_inline(
            ...
            extra_cuda_cflags=extra_hip_cflags,
        ...
        if self.save_kernels:
            combined = f"// === HIP kernels + launchers ===\n{hip_src}\n\n// === C++ bindings ===\n{cpp_src}"
